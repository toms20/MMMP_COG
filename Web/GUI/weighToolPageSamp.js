/*******************************************/
// This code is used to produce sample data
// for display purposes only. The code on GitHub
// is set up for actual use.

var mainWeight = 300;
var weightVariance = mainWeight*.05;
...
function randomWeight() {
	sampleData['weight'] = weightVariance*Math.random()+mainWeight;
}

function randomDistros() {
	sampleData['fb_dist'] = rearDist+((Math.random()-Math.random())*weightVariance/400);
	sampleData['lr_dist'] = leftDist+((Math.random()-Math.random())*weightVariance/600);
}

function randomLoads() {
	sampleData['lc1'] = sampleData['weight']*(1-sampleData['fb_dist'])*(1-sampleData['lr_dist']);
	sampleData['lc2'] = sampleData['weight']*(sampleData['fb_dist'])*(1-sampleData['lr_dist']);
	sampleData['lc3'] = sampleData['weight']*(sampleData['fb_dist'])*(sampleData['lr_dist']);
	sampleData['lc4'] = sampleData['weight']*(1-sampleData['fb_dist'])*(sampleData['lr_dist']);
}
...
function updateGUI() {
	var data = {'pin':96, 'value':sampleData['lc4']};
	onLoadCellData(data);
	data = {'pin':97, 'value':sampleData['lc1']};
	onLoadCellData(data);
	data = {'pin':98, 'value':sampleData['lc2']};
	onLoadCellData(data);
	data = {'pin':99, 'value':sampleData['lc3']};
	onLoadCellData(data);
}
...
function btnConnectClicked() {
	if(myCOMObject.isConnected) {
		...
		window.clearInterval(sampleDataInterval);
		myCOMObject.isConnected = false;
		...
		// }, 3000);
		sampleDataInterval = window.setInterval(function(){randomizeData()}, 500);
		document.getElementById('toolBarButtonTwo').innerHTML = 'Disconnect';
		myCOMObject.isConnected = true;
	}
...
function updateDistros() {
	if(totWeight > 25) {
		...
		if(isMetric) myWeightDistSideView.updateDistributionValues(1-avgFBDist, totWeight*lb_to_kg, isMetric);
		...
		if(isMetric) myWeightDistFrontView.updateDistributionValues(avgLRDist, totWeight*lb_to_kg, isMetric);
...
function updateCOGGraphics() {
	myTiltedRightWheelchair.updateCogValues(7+0.5*Math.random());
	myTiltedLeftWheelchair.updateCogValues(7+0.5*Math.random());
	myTiltedBackWheelchair.updateCogValues(16+1*Math.random());
}

window.addEventListener('load', initWeighToolPage, false);
window.addEventListener('unload', function() {myCOMObject.closeWebSocket()}, false);
window.onbeforeunload = function() {myCOMObject.closeWebSocket()};
