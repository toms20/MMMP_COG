...
bool WebSocket::doHandshake() {
    ...
    while ((bite = client.read()) != -1) {
        ...
        if (bite == '\n') { // EOL got, temp should now contain a header string
            ...
            if(strstr(temp, "GET /")) {
                firstHeader[counter - 2] = 0;
                fileServeTest = true;
                #ifdef DEBUG
                Serial.print("First Header:");
                Serial.println(firstHeader);
                #endif
                firstHeader2 = String(firstHeader);
            } else {
            ...
            if (!hasUpgrade && strstr(temp, "Upgrade:")) {
                ...
                #ifdef DEBUG
                Serial.print("hasKey with value:");
                Serial.println(key);
                #endif
                myKey = (String)key;
    ...
    if(!hasUpgrade && fileServeTest) {
        Serial.println("Twas a regular GET request (Not a web socket initiation!)");
        Serial.println(firstHeader2);
//        firstHeader2 = firstHeader2.substring(firstHeader2.indexOf("GET ")+4, firstHeader2.indexOf("HTTP/"));
        char firstHeader[firstHeader2.length()+1];
        firstHeader2.toCharArray(firstHeader, firstHeader2.length()+1);
        if(onServeFileRequested) onServeFileRequested(*this, client, firstHeader);
    }
    ...
    if (hasUpgrade && hasConnection && isSupportedVersion && hasHost && hasOrigin && hasKey) {
        Serial.print("Key:");Serial.println(myKey);
        key[80];
        myKey.toCharArray(key, myKey.length()+1);
        ...
        client.print(CRLF);
#ifdef DEBUG
        Serial.print("Final key:");Serial.println(temp);
#endif  
    } else {
...
void WebSocket::registerServeFileCallback(ServeFileCallback *callback) {
    onServeFileRequested = callback;
}
